...
* Installation
  ...
  Go-template source files ending in ~.tmpl~ or beginning with ~modify_~ use
  ~go-template-ts-mode~ directly when they have no host mode, or
  ~poly-any-go-template~ when the target file has one. Install the packages
  from [[https://github.com/chuxubank/poly-any-template][poly-any-template]] alongside this package.
...
make compile-extensions
  ...
  ~compile-all-extensions~ additionally checks the optional Company and Magit
  integrations when those packages are installed.
...
** Completion at point
   Standard completion-at-point integration is provided through ~chezmoi-capf~.
   It works with completion UIs such as Corfu and Company without depending on
   Cape internals. Add it to ~completion-at-point-functions~ to complete
   template variables.
