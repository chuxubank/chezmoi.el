...
* Installation
  ...
  located below ~.chezmoitemplates~ use ~go-template-ts-mode~ directly when
  they have no host extension, or ~poly-any-go-template~ when they have one.
  Inside ~.chezmoitemplates~, the optional ~.tmpl~ suffix is removed before
  checking for a host extension. Install the packages from
  [[https://github.com/chuxubank/poly-any-template][poly-any-template]] alongside this package.
